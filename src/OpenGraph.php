...
/**
 * Class OpenGraph
 * @description based on scottmac opengraph class, modification with charset detection and php xml hacks
 */
class OpenGraph implements Iterator
{
    ...
    static public function fetch($URI)
    {
        ...
        //curl_setopt($curl, CURLOPT_URL, rawurlencode($URI));
        ...
        $headers  = self::parseHeaders($headers_string);
     ...
     * @return array
     */
    static public function parseHeaders($headers_string)
    {
        $headers = explode("\n", $headers_string);
        $data = [];
     ...
     * @return bool
     */
    static public function getCharset($headers)
    {
        $content_type = $headers['Content-Type'];
        $t = explode(";", $content_type);
        if (is_array($t))
        {
            foreach ($t as $p)
            {
    ...
    static private function _parse($HTML, $headers)
    {
        ...
        if (!$detected)
        {
            ...
            preg_match('/<meta http-equiv="content-type"/i', $HTML, $m);

            // if ( strpos($HTML, '<meta http-equiv="content-type"') === false ) :
            if (!isset($m[0]))
            {
                $HTML = preg_replace( '/<head(.*?)>/i', '<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">', $HTML );//die("12312");
            }
            else
            {
